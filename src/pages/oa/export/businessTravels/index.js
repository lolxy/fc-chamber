...
export default {
  ...
  methods: {
    ...
    onExportSubmit() {
      ...
      Vue.$http.get('../v1/BusinessTravels?fields=Attachments,Checkers,Copyers,Type,Publisher', {
        params: formDatas
      }).then((response) => {
    ...
    checkRoles(empid) {
      Vue.$http.get(`../v1/OAInRoles/${empid}`).then((response) => {
        const roles = response.data.ObjRoles.filter(item => item === 'businesstravel');
        if (roles.length) {
  ...
  mounted() {
    oaCheckRoles(this.meId, 'businesstravel', this);
