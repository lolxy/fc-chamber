...
export default {
  ...
  methods: {
    ...
    onExportSubmit() {
      ...
      Vue.$http.get('../v1/OverTimes?fields=Attachments,Checkers,Copyers,Type,Publisher', {
        params: formDatas
      }).then((response) => {
    ...
    checkRoles(empid) {
      Vue.$http.get(`../v1/OAInRoles/${empid}`).then((response) => {
        const roles = response.data.ObjRoles.filter(item => item === 'overtime');
        if (roles.length) {
  ...
  mounted() {
    oaCheckRoles(this.meId, 'overtime', this);
