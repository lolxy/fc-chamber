...
export default {
  ...
  methods: {
    ...
    onExportSubmit() {
      ...
      Vue.$http.get('../v1/Purchases?fields=Attachments,Checkers,Copyers,Type,Publisher', {
        params: formDatas
      }).then((response) => {
    ...
    checkRoles(empid) {
      Vue.$http.get(`../v1/OAInRoles/${empid}`).then((response) => {
        const roles = response.data.ObjRoles.filter(item => item === 'purchase');
        if (roles.length) {
  ...
  mounted() {
    oaCheckRoles(this.meId, 'purchase', this);
